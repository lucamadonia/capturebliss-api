...
import com.capturebliss.api.common.*;
import com.capturebliss.api.config.PaymentConfig;
import com.capturebliss.api.entity.*;
import com.capturebliss.api.repo.EntityConfigKVRepo;
import com.capturebliss.api.repo.OrgRepo;
import com.capturebliss.api.repo.SubscriptionRepo;
import com.capturebliss.api.service.vendor.SlackMsgService;
import com.capturebliss.api.transport.NfEvents;
import com.capturebliss.api.transport.PaymentTerms;
import com.capturebliss.api.transport.req.ReqDeductCredit;
import com.capturebliss.api.transport.req.ReqSubscriptionInfo;
import com.capturebliss.api.transport.req.ReqUpdateSubInfo;
import com.capturebliss.api.transport.resp.RespSubsValidation;
...
public class SubscriptionService {
  ...
  private static final String CAPTUREBLISS_GIVEN_CREDIT = "CAPTUREBLISS_GIVEN_CREDIT";
  ...
  public static int computeAvailableCredit(List<EntityConfigKV> entityConfigKV) {
    ...
    for (EntityConfigKV configKV : entityConfigKV) {
      ...
      if (StringUtils.equalsIgnoreCase(CAPTUREBLISS_GIVEN_CREDIT, configKV.getConfigKey())) totalCredit += info.getValue();
  ...
  public List<EntityConfigKV> setCreditsForOrg(Subscription subscription, int userCount, boolean shouldResetUsage) {
    List<EntityConfigKV> existingCredits = entityConfigKVRepo.findEntityConfigKVSByEntityTypeAndEntityIdAndConfigTypeAndConfigKey(
      ...
      CAPTUREBLISS_GIVEN_CREDIT
    );
    List<EntityConfigKV> updatedCredits = new ArrayList<>();

    PaymentConfig.CreditValue creditValue = determineCaptureblissCredits(subscription);
    ...
    if (existingCredits.isEmpty()) {
      ...
      EntityConfigKV captureblissGivenCredit = existingCredits.get(0);
      CreditInfo creditInfo = mapper.convertValue(captureblissGivenCredit.getConfigVal(), CreditInfo.class);
      creditInfo.setValue(creditScaledByUserCount);
      creditInfo.setAbsValue(creditScaledByUserCount);
      creditInfo.setUpdatedAt(Utils.getCurrentUtcTimestamp());
      captureblissGivenCredit.setConfigVal(creditInfo);
      ...
      if (shouldResetUsage) {
        ...
        // During reset we only reset capturebliss given credit
        // Let's say capturebliss_given_credit = 100 & top_up_credit = 50 and user is used up 75 credits. Since the credit will
        // first go from capturebliss_given_credit hence during usage reset we would do max(0, 75 - 100) = 0
        // Now let's say user capturebliss_given_credit = 100 & top_up_credit = 50 and user is used up 125 credits
        // i.e. user has used up all capturebliss given credit and then used up 25/50 user credit, so we would keep the usage
      ...
      updatedCredits.add(captureblissGivenCredit);
    }
  ...
  private List<EntityConfigKV> setCreditsForOrgWhenCreditsIsEmpty(Subscription subs, int credits) {
    ...
    EntityConfigKV captureblissGivenCredit = EntityConfigKV.builder()
      .entityId(subs.getOrgId())
      .entityType(ConfigEntityType.Org)
      .configType(EntityConfigConfigType.AI_CREDIT)
      .configKey(CAPTUREBLISS_GIVEN_CREDIT)
    ...
    entityConfigKVS.add(captureblissGivenCredit);
    entityConfigKVS.add(topUpCredit);
    entityConfigKVS.add(creditUsed);
    return entityConfigKVS;
  }

  private PaymentConfig.CreditValue determineCaptureblissCredits(Subscription subscription) {
  ...
  public RespSubscription updateCreditAfterDeduction(ReqDeductCredit req, User user) {
    ...
    EntityConfigKV captureblissCredit = creditDetails.stream().filter(credit -> credit.getConfigKey().equals(CAPTUREBLISS_GIVEN_CREDIT)).findFirst().get();
    ...
    CreditInfo captureblissCreditInfo = mapper.convertValue(captureblissCredit.getConfigVal(), CreditInfo.class);
    CreditInfo topUpCreditInfo = mapper.convertValue(topUpCredit.getConfigVal(), CreditInfo.class);
    CreditInfo usedCreditInfo = mapper.convertValue(usedCredit.getConfigVal(), CreditInfo.class);

    // current normalized credit usage can't go above capturebliss credit info + topup credit info
    ...
    int totalCredit = captureblissCreditInfo.getValue() + topUpCreditInfo.getValue();
    usedCreditInfo.setAbsValue(currNValue);
    usedCreditInfo.setValue(Math.min(currNValue, totalCredit));
    usedCredit.setConfigVal(usedCreditInfo);

    updatedCreditDetails.add(captureblissCredit);
  ...
  @Transactional
  public RespSubscription migrateCaptureblissCredit(Long orgId) {
