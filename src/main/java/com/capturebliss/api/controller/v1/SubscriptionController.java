...
import com.capturebliss.api.config.AppSettings;
import com.capturebliss.api.config.PaymentConfig;
import com.capturebliss.api.entity.User;
import com.capturebliss.api.service.SubscriptionService;
...
public class SubscriptionController {
  ...
  @RequestMapping(value = Routes.REFILL_CAPTUREBLISS_CREDIT, method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
  public ApiResp<RespSubscription> refillCaptureblissCredit(@PathVariable("org_id") Long orgId) {
    if (!appSettings.isDataEntryFlagSet()) {
      log.error("Data entry requested but flag not set but REFILL_CAPTUREBLISS_CREDIT for {} is requested.", orgId);
  ...
  @RequestMapping(value = Routes.MIGRATE_CAPTUREBLISS_CREDIT, method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
  public ApiResp<RespSubscription> migrateCaptureblissCredit(@PathVariable("org_id") Long orgId) {
    if (!appSettings.isDataEntryFlagSet()) {
      log.error("Data entry requested but flag not set but REFILL_CAPTUREBLISS_CREDIT for {} is requested.", orgId);
      throw new ResponseStatusException(HttpStatusCode.valueOf(404));
    }
    RespSubscription subs = subsService.migrateCaptureblissCredit(orgId);
    return ApiResp.<RespSubscription>builder().status(ApiResp.ResponseStatus.Success).data(subs).build();
  }
